...
const OrderList: React.FC = () => {
  ...
  return (
    <div className="px-7 py-5  mr-5 h-screen">
      <Helmet>
      ...
      <div className=" w-full mt-8">
        <div className="flex w-full flex-col items-center justify-between space-y-2 sm:flex-row sm:space-y-0">
          ...
          <div className="relative inline-flex">
            <select
              className="border text-black border-gray-300 rounded px-2 py-1 text-sm outline-none focus:border-blue-500"
        ...
        <table className="min-w-full border-collapse border-spacing-y-2 border-spacing-x-2">
          <thead className="hidden border-b lg:table-header-group">
            <tr>
              ...
              <th className="lg:px-6 py-3 text-left text-xs font-medium  uppercase tracking-wider">
                Total Amount
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium  uppercase tracking-wider">
                last payment
              ...
              filteredOrders.map(order => (
                <tr key={order._id}>
                  <td className="whitespace-no-wrap py-4 text-left text-sm  sm:px-3 lg:text-left">
                    <div className="sm:text- text-xs font-medium ">
                      {order.user.firstName}
                    </div>
                    <div className="sm:text-sm text-xs">{order.user.email}</div>
                  </td>
                    ...
                    <div className="grid lg:grid-cols-3 grid-cols-1">
                      {order.items
                        ...
                        .map(item => (
                          <div
                            key={item._id}
                            className="flex items-center space-x-5 space-y-6"
                          >
                            <img
                              src={item.image}
                              alt={item.name}
                            ...
                            />
                            <div className="text-xs  cursor-pointer hidden sm:inline">
                      ...
                      {order.items.length > 3 && (
                        <div
                          className="text-blue-500 md:text-start text-center mt-3 cursor-pointer items-center"
                  ...
                  <td className="whitespace-no-wrap py-4 text-right hidden sm:block text-sm sm:px-3 lg:text-left">
                    {order.totalAmount} $
                  </td>
                  <td className="whitespace-no-wrap hidden py-4 text-sm font-normal text-gray-500 sm:px-3 sm:table-cell">
      ...
      {expandedOrder && (
        ...
        p-8 max-w-xl mx-auto rounded shadow-lg max-h-96
        sm:max-w-md
      `}
          >
            <div>
                      ...
                      <div>
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: highlightSearchQuery(item.name),
                          }}
                        >
                          {item.name}
                        </div>
